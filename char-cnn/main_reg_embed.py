...
'''
Parameters:
python main_reg.py maxlen nb_filter dense_outputs filter_size pooling_size testing
'''
...
#Model params
embed_dim = 30
nb_filter = 128 #int(sys.argv[2]) # 256, Number of filters for conv layers
...
save = True
load = True
model_name_path = 'save/model' + string.join(sys.argv[1:], '-') + '-emb.json'
model_weights_path = 'save/model_weights' + string.join(sys.argv[1:], '-') + '-emb.h5'
...
test_data = data_helpers.encode_data_idx(x_test, maxlen, vocab, vocab_size, check) # no _idx for one-hot

print('Build model...')

model = py_crepe_reg_embed.model(filter_kernels, dense_outputs, maxlen, vocab_size, nb_filter, pooling, embed_dim)
...
while not STOP:
    ...
    if save and STOP:
        ...
        with open("pred-" + string.join(sys.argv[1:], '-') + "-emb.csv", "w") as f:
        ...
        # Accuracy
        with open("test-acc-" + string.join(sys.argv[1:], '-') + "-emb.txt", "w") as f:
