...
dense_outputs = 64 #300 # Number of units in the dense layer
#filter_kernels = [10, 7, 3, 3, 3, 3]
filter_kernels = [64,32,16,8,4,2] #Conv layer kernel size
pooling = 4 # pooling over
...
model = py_crepe_charnn.model(filter_kernels, dense_outputs, maxlen, vocab_size, nb_filter, pooling, embed_dim)
...
while not STOP:
    ...
    if save and STOP:
        ...
        with open("pred-" + string.join([str(f) for f in filter_kernels], '-') + "-cnn.csv", "w") as f:
        ...
        # Accuracy
        with open("test-acc-" + string.join([str(f) for f in filter_kernels], '-') + "-cnn.txt", "w") as f:
